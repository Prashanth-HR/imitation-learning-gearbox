...
class AutomaticDemoCollector:
    ...
    def __init__(self, task_name, total_num_trajectories, num_timesteps_per_image, max_velocity_scale, max_accleration_scale, demo_or_target='demo'):
        ...
        self.max_accleration_scale = max_accleration_scale
    ...
    def run(self):
        ...
        while self.is_ros_running and trajectory_num < self.total_num_trajectories:
            ...
            init_theta_y = config.DEMO_START_MID_ORI[1] + theta_sign * theta_width_ratio * config.TASK_SPACE_ANGLE
            init_theta_z = config.DEMO_START_MID_ORI[2] + theta_sign * theta_width_ratio * config.TASK_SPACE_ANGLE
            ...
            # Save the images
            # commented so that the image list was consuming more space
    ...
    def _move_to_target_and_record_data(self, target_pose):
        ...
        while self.is_ros_running:
            ...
            if i==0:
                print('#### Move to Traget pose ####')
                robot_exe_thread.start()
            i+=1
            #has_reached_goal = self.sawyer.move_towards_pose(target_pose, self.max_velocity_scale, self.max_accleration_scale)
            # Sleep until the next loop
            self.ros_rate.sleep()
            # Update the control time step
            self.control_time_step += 1
            # If the endpoint has reached the goal, break
            if not robot_exe_thread.is_alive():
                break
